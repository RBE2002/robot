...
class Drivetrain : public Loop {
 public:
  ...
  /**
    ...
    * The Record feature was not actually used on the final robot.
    */
  struct Record {
    Direction heading;
    double dist;  // meters.
  ...
  void Stop(bool resume = false);  // Stops the robot; ends the current portion
                                   // of the path.

  void Run();  // Overrides Run from Loop class.

  // Sets whether or not we should be wall following.
  void set_wall_follow(bool wall_follow) { wall_follow_ = wall_follow; }

  // Sets whether or not we should be navigating (eg, doing cliff detection,
  // etc.)
  void set_navigating(bool navigate) { navigating_ = navigate; }

  // Whether to wall follow on left or right, used for reversing path to start.
  ...
  /**
    * Overrides Loop::Update() and updates all members which also need to be
   * called periodically.
    */
  void Update() {
    Loop::Update();
  ...
  // Returns true when we have completed a DriveDist move.
  bool drive_dist_done() { return drive_dist_done_; }

  // Returns true if robot is in process of stopping.
  bool stopping() { return stopping_; }

  // Sets information for printing on LCD display.
  void set_z(double z) { z_pos_ = z; }
  void set_found(bool found) { found_ = found; }
  // Affects the displayed candle position, as an offset must be added to the
  // recorded robot position in the direction of the candle, or else we will
  // display the position of the robot, not the candle.
  void set_candle_dir(Direction candle_dir) { candle_dir_ = candle_dir; }
  // Return absolute position of robot relative to start.
 ...
 private:
  ...
  bool finv_, linv_, binv_,
      rinv_;  // inversions for if the motors are installed in reverse
  Encoder fenc_, lenc_, benc_,
      renc_;  // declares all the Encoders for later ease
  // Range sensors; in order, they are the sensors pointing forwards, left,
  // back, and right.
  Range range_[kNumMotors];
  CliffDetector
      cliff_;  // this variable represents the state of all the line sensors
  ...
  // Microseconds
  unsigned long
      prev_time_;  // a previous time for taking averages and derivatives
